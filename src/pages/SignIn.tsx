  ...
  return (
    <main className="safe-top safe-bottom mx-auto flex h-full max-w-md flex-col justify-between px-6 pb-8">
      <div className="pt-[18vh]">
        ...
        <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ ...spring, delay: 0.18 }} className="mt-3 text-lg text-muted">
          Train with grit.
        </motion.p>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ ...spring, delay: 0.26 }} className="space-y-3">
        ...
        {!sync.online && <p className="rounded-2xl bg-surface-2 p-3 text-sm text-muted">Offline. Signing in needs a connection.</p>}
