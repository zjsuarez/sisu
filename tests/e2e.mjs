...
await phone.ev(`window.__msg = ''; window.confirm = (m) => { window.__msg = m; return true }`)
await phone.ev(click('Finish'))
await sleep(400)
check('finishing warns about sets that were never ticked', /not ticked/i.test(await phone.ev('window.__msg')), await phone.ev('window.__msg'))
await sleep(2500)
