...
// ---- zen mode: one exercise at a time, swipe for the next
await phone.ev(`document.querySelector('[aria-label="Zen mode"]').click()`)
await sleep(1200)
const zenTitle = () => phone.ev(`(() => {
  const s = document.querySelector('.snap-x')
  const i = Math.round(s.scrollLeft / s.clientWidth)
  return s.children[i]?.querySelector('h1')?.textContent ?? 'NO PAGE'
})()`)
check('zen mode shows one exercise, full screen', (await zenTitle()) === 'Bench Press', await zenTitle())
check('the chronometer is there, stopped', !!(await phone.ev(`!!document.querySelector('[aria-label="Start timer"]')`)))
await phone.shot('e2e-zen')
await phone.ev(`(() => { const s = document.querySelector('.snap-x'); s.scrollBy({ left: s.clientWidth }); return 'swiped' })()`)
await sleep(1000)
check('swiping moves to the next exercise', (await zenTitle()) === 'Overhead Press', await zenTitle())
await phone.ev(`document.querySelector('[aria-label="Exercise options"]').click()`)
await sleep(900)
await phone.ev(`[...document.querySelectorAll('[role="dialog"] button')].find((b) => b.textContent.trim() === 'History')?.click()`)
await sleep(900)
check('the menu can show what you lifted before', /Nothing logged yet|×/.test(await phone.ev(`document.querySelector('[role="dialog"]')?.innerText ?? ''`)), await phone.ev(`document.querySelector('[role="dialog"]')?.innerText.slice(0, 60) ?? 'NO SHEET'`))
await phone.ev(closeSheet())
await sleep(700)
await phone.ev(`document.querySelector('[aria-label="Leave zen mode"]').click()`)
await sleep(1000)
